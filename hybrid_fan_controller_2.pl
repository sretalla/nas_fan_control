...
$debug = 0;
$debug_log = '/mnt/vol2/scripts/PID_fan_control_debug.log';

## LOG
$log = '/mnt/vol2/scripts/PID_fan_control.log';
...
$high_cpu_temp = 50;       # will go HIGH when we hit
...
$hd_fan_duty_low       = 45;    # some 120mm fans stall below 30.
...
$cpu_fan_header = "FAN 4";                 # used for printing to standard output for debugging   
$hd_fan_header  = "FAN 1";                 # used for printing to standard output for debugging   
@hd_fan_list = ("FAN 1", "FAN 2", "FAN 3");  # used for logging to file  
...
## The script needs to know where OpenCorsairLink is
$opencorsairlink = "/mnt/vol2/scripts/OpenCorsairLink3-pin.elf";
$use_OCL = 1;
...
sub get_hd_list
{
    my $disk_list = `camcontrol devlist | grep -v "SSD" | grep -v "Verbatim" | grep -v "KINGSTON" | grep -v "Virtual" | sed 's:.*(::;s:).*::;s:,pass[0-9]*::;s:pass[0-9]*,::' | egrep '^[a]*da[0-9]+\$' | tr '\012' ' '`;
    dprint(3,"$disk_list\n");
...
sub set_fan_mode
{
    ...
    if( $use_OCL == 1 )
    {
        # Don't need to do anything here unless mode is full
        dprint( 1, "Setting fan mode to $mode ($fan_mode)\n");
        if( $fan_mode eq 'full' )
        {
            `$opencorsairlink --device 0 --fan channel=0,mode=0,pwm=100`;
            `$opencorsairlink --device 0 --fan channel=1,mode=0,pwm=100`;
            `$opencorsairlink --device 0 --fan channel=2,mode=0,pwm=100`;
            `$opencorsairlink --device 0 --fan channel=3,mode=0,pwm=100`;
            `$opencorsairlink --device 0 --fan channel=4,mode=0,pwm=100`;
            #`$opencorsairlink --device 0 --fan channel=5,mode=0,pwm=100`;
        }
    }
    else
    {
...
sub set_fan_zone_duty_cycle
{
    ...
    if( $use_OCL == 1 )
    {
        dprint( 0, "Set FAN used Zone: $zone and Duty: $duty.\n");
        if( $zone == 0 )
        {
            `$opencorsairlink --device 0 --fan channel=3,mode=0,pwm=$duty`;
            `$opencorsairlink --device 0 --fan channel=4,mode=0,pwm=$duty`;
            #$cpu_fake = $cpu_max_fan_speed * ( $duty / 100 );
            #dprint( 0, "CPU Fan Zone faked to $cpu_fake\n");
        }
        elsif( $zone == 1 )
        {
            `$opencorsairlink --device 0 --fan channel=0,mode=0,pwm=$duty`;
            `$opencorsairlink --device 0 --fan channel=1,mode=0,pwm=$duty`;
            `$opencorsairlink --device 0 --fan channel=2,mode=0,pwm=$duty`;
            #$HD_fake = $hd_max_fan_speed * ( $duty / 100 );
            #dprint( 0, "HD Fan Zone faked to $HD_fake\n");
        }
        #`$opencorsairlink --device 0 --fan channel=$zone,mode=0,pwm=$duty`;
    }
    else
    {
...
sub get_fan_speed
{
    ...
    if( $use_OCL == 1 )
    {
        $command = "$opencorsairlink --device 0 --fan channel=0,mode=0 | grep '$fan'";
    }
    else
    {
        $command = "$ipmitool sdr | grep '$fan'";
    }
    ...
    if( $fan_speed eq "no" )
    {
        dprint( 0, "$fan Fan speed: No reading\n");
        $fan_speed = -1;
    }
    elsif( $fan_speed eq "disabled" )
    {
        dprint( 0, "$fan Fan speed: Disabled\n");
        $fan_speed = -1;

    }
    elsif( $fan_speed > 10000 || $fan_speed < 0 )
    {
        dprint( 0, "$fan Fan speed: $fan_speed RPM, is nonsensical\n");
        $fan_speed = -1;
    }
    else    
    {
        dprint( 1, "$fan Fan speed: $fan_speed RPM\n");
    }
...
# get fan speed for specified fan header
{
    ...
    if( $use_OCL == 1 )
    {
        $command = "$opencorsairlink --device 0 --fan channel=0,mode=0 | grep '$fan_name'";
    }
    else
    {
        $command = "$ipmitool sdr | grep '$fan_name'";
    }
    my $output = `$command`;
    my @vals = split(" ", $output);
    my $fan_speed = "$vals[2]";
